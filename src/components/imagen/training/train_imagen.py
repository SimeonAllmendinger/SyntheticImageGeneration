...
from src.components.imagen.data_manager.imagen_dataset import CholecT45ImagenDataset, CholecSeg8kImagenDataset
...
def train_imagen(opt: Opt, imagen_model: Imagen):
    ...
    train_valid_split=[imagen_model.OPT_IMAGEN_TRAINER['train_split'], imagen_model.OPT_IMAGEN_TRAINER['valid_split']]
    ...
    train_generator = DataLoader(dataset=train_dataset, 
                                  batch_size=imagen_model.OPT_IMAGEN_TRAINER['batch_size'], 
                                  shuffle=imagen_model.OPT_IMAGEN_TRAINER['shuffle']
                                )
    ...
    model_save_path=imagen_model.OPT_IMAGEN_TRAINER['PATH_MODEL_SAVE']
    model_checkpoint_path=imagen_model.OPT_IMAGEN_TRAINER['PATH_MODEL_CHECKPOINT']
    
    # Make results folder with timestamp
    ...
    unet_number=imagen_model.OPT_IMAGEN_TRAINER['unet_number']
    
    for epoch in tqdm(range(1, imagen_model.OPT_IMAGEN_TRAINER['max_epochs'] + 1)):
        ...
        if not (epoch % 1000) and imagen_model.trainer.is_main:
            ...
            sample_text = imagen_dataset.df_triplets['FRAME TRIPLET TEXT'].values[sample_index]
