...
def train_imagen(opt: Opt, imagen_model: Imagen):
    ...
    for epoch in tqdm(range(1, opt.imagen['trainer']['max_epochs'] + 1)):
        ...
        if not (epoch % 1000) and imagen_model.trainer.is_main:
            ...
            sample_index = np.random.randint(low=0,high=551)
            
            sample_image, sample_text_embeds = imagen_dataset.__getitem__(sample_index)
            sample_text = imagen_dataset.df_triplets['triplet_text'].values[sample_index]
