...
parser.add_argument('--accelerate', type=str, default=False,  help='Usage of accelerate to enhance multiprocessing on multiple gpus')

FLAGS, unparsed = parser.parse_known_args()

# Start run with neptune docs
_opt_=Opt()
cond_scale = _opt_.rendevouz['synthetic_data']['cond_scale']
model_type = _opt_.rendevouz['synthetic_data']['model']
max_synthetic_images_list = _opt_.rendevouz['synthetic_data']['max_synthetic_images_list']
only_train_synthetic = _opt_.rendevouz['synthetic_data']['only_train_synthetic']
neptune_ai = Neptune_AI(opt=_opt_)
neptune_ai.start_neptune_run(_opt_)
...
use_accelerate  = FLAGS.accelerate
...
model_dir       = "/home/kit/stud/uerib/SyntheticImageGeneration/results/rendevouz/run_{}".format(version)
if not os.path.exists(model_dir): os.makedirs(model_dir)
resume_ckpt     = None


for max_synthetic_images in max_synthetic_images_list:
    
    ckpt_path       = os.path.join(model_dir, f'{model_type}-cs_{cond_scale}-k_{kfold}-{modelname}-{max_synthetic_images}-{only_train_synthetic}.pth')
    logfile         = os.path.join(model_dir, f'{model_type}-cs_{cond_scale}-k_{kfold}-{modelname}-{max_synthetic_images}-{only_train_synthetic}.log')
    ...
    def train_loop(dataloader, model, activation, loss_fn_i, loss_fn_v, loss_fn_t, loss_fn_ivt, optimizers, scheduler, epoch):
        start = time.time() 
        for batch, (img, (y1, y2, y3, y4)) in enumerate(tqdm(dataloader)):
            ...
            neptune_ai.log_neptune_run(opt=_opt_,
                                        data_item=loss,
                                        neptune_run_save_path=f"training/loss_RDV")
    ...
    dataset = dataloader.CholecT50( 
                ...
                max_rows=max_synthetic_images,
                only_train_synthetic=only_train_synthetic
                )
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, prefetch_factor=3*batch_size, num_workers=8, pin_memory=True, persistent_workers=True, drop_last=False)
    val_dataloader   = DataLoader(val_dataset, batch_size=batch_size, shuffle=True, prefetch_factor=3*batch_size, num_workers=8, pin_memory=True, persistent_workers=True, drop_last=False)
    ...
    #%% model
    model = network.Rendezvous('resnet18', hr_output=hr_output, use_ln=use_ln)
    if not use_accelerate:
        model = model.cuda()
    ...
    if use_accelerate:
        loss_fn_i   = nn.BCEWithLogitsLoss(pos_weight=torch.tensor(tool_weight))
        loss_fn_v   = nn.BCEWithLogitsLoss(pos_weight=torch.tensor(verb_weight))
        loss_fn_t   = nn.BCEWithLogitsLoss(pos_weight=torch.tensor(target_weight))
    else:
    ...
    #%% log config
    header1 = "** Run: {} | Framework: PyTorch | Method: {} | Version: {} | Model: {} | #Synthetic: {} | Cond_Scale: {} | Data: CholecT50 | Batch: {} **".format(os.path.basename(__file__), modelname, version, model_type, max_synthetic_images, cond_scale, batch_size)
        ...
        for epoch in tqdm(range(0,epochs)):
            try:
                ...
                if epoch % val_interval == 0:
                    ...
                    # Upload epoch valid loss to neptune_ai
                    neptune_ai.log_neptune_run(opt=_opt_,
                                                data_item=mAP.compute_video_AP('ivt', ignore_null=set_chlg_eval)['mAP'],
                                                neptune_run_save_path=f"training/ivt_RDV")
...
neptune_ai.stop_neptune_run(opt=_opt_)
